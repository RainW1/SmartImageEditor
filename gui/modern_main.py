...
class ModernImageEditor(ctk.CTk):
    ...
    def show_adjust_panel(self):
        ...
        info = ctk.CTkLabel(
            self.control_panel,
            text="Real-time preview enabled",
            text_color="gray"
        )
        info.pack(pady=(0, 20))
        ...
        ctk.CTkLabel(brightness_header, text="Brightness", font=("Arial", 13)).pack(side="left")
        ...
        ctk.CTkLabel(contrast_header, text="Contrast", font=("Arial", 13)).pack(side="left")
        ...
        ctk.CTkLabel(saturation_header, text="Saturation", font=("Arial", 13)).pack(side="left")
...
# Replace show_ai_panel() method with this complete version
    ...
    def ai_color_correction_dialog(self):
        ...
        title = ctk.CTkLabel(
            dialog,
            text="⚙️ Custom AI Color Correction",
            font=("Arial", 18, "bold")
        )
        title.pack(pady=20)
        ...
        # CLAHE Clip Limit
        ctk.CTkLabel(settings_frame, text="CLAHE Clip Limit (1.0 - 5.0):", font=("Arial", 12)).pack(pady=(10,5), anchor="w")
        clip_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        clip_frame.pack(fill="x", pady=5)
        
        clip_value_label = ctk.CTkLabel(clip_frame, text="3.0", font=("Arial", 12, "bold"), text_color="#3b82f6")
        clip_value_label.pack(side="right")
        
        clip_slider = ctk.CTkSlider(settings_frame, from_=1.0, to=5.0, number_of_steps=40)
        clip_slider.set(3.0)
        clip_slider.pack(fill="x", pady=5)
        clip_slider.configure(command=lambda v: clip_value_label.configure(text=f"{v:.1f}"))
        
        # Tile Size
        ctk.CTkLabel(settings_frame, text="Tile Grid Size (2 - 16):", font=("Arial", 12)).pack(pady=(10,5), anchor="w")
        tile_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        tile_frame.pack(fill="x", pady=5)
        
        tile_value_label = ctk.CTkLabel(tile_frame, text="8", font=("Arial", 12, "bold"), text_color="#3b82f6")
        ...
        tile_slider.configure(command=lambda v: tile_value_label.configure(text=str(int(v))))
        
        # Gamma
        ctk.CTkLabel(settings_frame, text="Gamma (0.5 - 3.0):", font=("Arial", 12)).pack(pady=(10,5), anchor="w")
        ctk.CTkLabel(settings_frame, text="< 1.0 = Brighter | > 1.0 = Darker", font=("Arial", 9), text_color="gray").pack(anchor="w")
        gamma_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        gamma_frame.pack(fill="x", pady=5)
        
        gamma_value_label = ctk.CTkLabel(gamma_frame, text="1.0", font=("Arial", 12, "bold"), text_color="#3b82f6")
        gamma_value_label.pack(side="right")
        
        gamma_slider = ctk.CTkSlider(settings_frame, from_=0.5, to=3.0, number_of_steps=50)
        gamma_slider.set(1.0)
        gamma_slider.pack(fill="x", pady=5)
        gamma_slider.configure(command=lambda v: gamma_value_label.configure(text=f"{v:.1f}"))
        
        # Brightness
        ctk.CTkLabel(settings_frame, text="Brightness (-50 to 50):", font=("Arial", 12)).pack(pady=(10,5), anchor="w")
        brightness_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        brightness_frame.pack(fill="x", pady=5)
        
        brightness_value_label = ctk.CTkLabel(brightness_frame, text="0", font=("Arial", 12, "bold"), text_color="#3b82f6")
        brightness_value_label.pack(side="right")
        
        brightness_slider = ctk.CTkSlider(settings_frame, from_=-50, to=50, number_of_steps=100)
        brightness_slider.set(0)
        brightness_slider.pack(fill="x", pady=5)
        brightness_slider.configure(command=lambda v: brightness_value_label.configure(text=str(int(v))))
        
        # Contrast
        ctk.CTkLabel(settings_frame, text="Contrast (-50 to 50):", font=("Arial", 12)).pack(pady=(10,5), anchor="w")
        contrast_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        contrast_frame.pack(fill="x", pady=5)
        
        contrast_value_label = ctk.CTkLabel(contrast_frame, text="0", font=("Arial", 12, "bold"), text_color="#3b82f6")
        contrast_value_label.pack(side="right")
        
        contrast_slider = ctk.CTkSlider(settings_frame, from_=-50, to=50, number_of_steps=100)
        contrast_slider.set(0)
        contrast_slider.pack(fill="x", pady=5)
        contrast_slider.configure(command=lambda v: contrast_value_label.configure(text=str(int(v))))
    ...
    def ai_remove_background(self):
        ...
        try:
            ...
            self.update_status(f"✅ Background removed ({mode}) | {self.get_history_position()}")
    ...
    def ai_style_transfer_dialog(self):
        ...
        messagebox.showinfo(
            ...
            "Style Transfer feature is available but takes 30-60 seconds to process.\n\n" +
            "To use it:\n" +
            "1. Select a style image\n" +
            "2. Adjust intensity (0-1)\n" +
    ...
    def on_crop_end(self, event):
        ...
        if self.crop_start:
            ...
            self.update_status(f"✂️ Area selected: {x2-x1}×{y2-y1} px - Click Apply to crop")
    ...
    def apply_resize(self):
        ...
        try:
            ...
            if not width_str or not height_str:
                messagebox.showwarning("Invalid Input", "Please enter both width and height!")
                return
    ...
    def show_analysis_panel(self):
        ...
        messagebox.showinfo("Coming Soon", "Analysis panel will be implemented next!")
    ...
    def open_image(self):
        ...
        if path:
            ...
            if img is not None:
                ...
                self.display_image_on_canvas()
                self.update_status(f"Opened: {os.path.basename(path)} | {self.get_history_position()}")
            else:
    ...
    def apply_changes(self):
        ...
        if self.temp_brightness == 0 and self.temp_contrast == 0 and self.temp_saturation == 0:
    ...
    def undo_action(self):
        ...
        if self.history_index > 0:
            ...
            self.update_status(f"↶ Undo: {state['description']} | {self.get_history_position()}")
        else:
    ...
    def redo_action(self):
        ...
        if self.history_index < len(self.history) - 1:
            ...
            state = self.history[self.history_index]
            self.update_status(f"↷ Redo: {state['description']} | {self.get_history_position()}")
        else:
    ...
    def update_history_status(self):
        ...
        if len(self.history) > 0:
            state = self.history[self.history_index]
            self.update_status(f"✅ {state['description']} | {self.get_history_position()}")
